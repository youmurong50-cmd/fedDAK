...
def check(config_path, train_path, test_path, num_clients, niid=False, balance=True, partition=None, batch_size = 10, alpha=0.3):
...
def prepare_path(base_data_dir, data_dir, args):
    abs_data_dir = os.path.join(base_data_dir, data_dir)
    if not os.path.exists(abs_data_dir):
        os.makedirs(abs_data_dir)
    
    experiment_name = ""
    if args.partition == "dir":
        alpha_splits = str(args.alpha).split(".")
        alpha_str = alpha_splits[0] + "_" + alpha_splits[1]
        experiment_name = "{}_nc{}_{}{}_dir_alpha{}".format(
            data_dir,
            args.num_clients,
            "noniid" if args.noniid else "iid",
            "_balance" if args.balance else "",
            alpha_str
        )
    elif args.partition == "exdir":
        experiment_name = "{}_nc{}_{}{}_exdir_alpha{}".format(
            data_dir,
            args.num_clients,
            "noniid" if args.noniid else "iid",
            "_balance" if args.balance else "",
        ...
        experiment_name = "{}_nc{}_{}{}_pat".format(
            data_dir,
            args.num_clients,
            "noniid" if args.noniid else "iid",
            "_balance" if args.balance else ""
        ...
        raise NotImplementedError("unexpected partition strategy: {}".format(args.partition))
        
    # Setup directory for train/test data
    config_path = abs_data_dir + "/" + experiment_name + "/config.json"
    train_path = abs_data_dir + "/" + experiment_name + "/train/"
    test_path = abs_data_dir + "/" + experiment_name + "/test/"
    return experiment_name, config_path, train_path, test_path

def separate_data(data, num_clients, num_classes, niid=False, balance=False, partition=None, batch_size = 10, train_ratio=0.75, alpha=0.3, class_per_client=None):
...
def save_file(config_path, train_path, test_path, train_data, test_data, num_clients, 
                num_classes, statistic, niid=False, balance=True, partition=None, batch_size=10, alpha=0.3):
    config = {
    ...
    # gc.collect()
    print("Saving to disk, train_path: {}, test_path: {}, config_path: {}\n".format(
        train_path, test_path, config_path
    ))

    for idx, train_dict in enumerate(train_data):
        with open(train_path + str(idx) + '.npz', 'wb') as f:
            np.savez_compressed(f, data=train_dict, pickle_kwargs={'protocol': 4})
    for idx, test_dict in enumerate(test_data):
        with open(test_path + str(idx) + '.npz', 'wb') as f:
            np.savez_compressed(f, data=test_dict, pickle_kwargs={'protocol': 4})
...
def read_data(base_data_dir, dataset, experiment_name, idx, is_train=True):
    if is_train:
        train_data_dir = os.path.join(base_data_dir, dataset, experiment_name, 'train/')
        ...
        test_data_dir = os.path.join(base_data_dir, dataset, experiment_name, 'test/')
...
def read_client_data(base_data_dir, dataset, experiment_name, idx, is_train=True):
    if is_train:
        train_data = read_data(base_data_dir, dataset, experiment_name, idx, is_train)
        ...
        test_data = read_data(base_data_dir, dataset, experiment_name, idx, is_train)
